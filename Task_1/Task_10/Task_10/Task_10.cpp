...
void savePhoneDirectoryToFile() {
    std::ofstream outputFile("phone_directory.txt");
    if (outputFile.is_open()) {
        outputFile << currentRecords << std::endl;
        for (int i = 0; i < currentRecords; ++i) {
            outputFile << phoneDirectory[i].id << " "
                << phoneDirectory[i].lastName << " "
                << phoneDirectory[i].firstName << " "
                << phoneDirectory[i].middleName << " "
                << phoneDirectory[i].birthDate.day << " "
                << phoneDirectory[i].birthDate.month << " "
                << phoneDirectory[i].birthDate.year << " "
                //<< phoneDirectory[i].zodiacSign << " "
                << phoneDirectory[i].phoneNumber << std::endl;
        }
        outputFile.close();
        std::cout << "Телефонный справочник успешно сохранен в файл." << std::endl;
    }
    else {
        std::cout << "Ошибка открытия файла для сохранения." << std::endl;
    }
}
void loadPhoneDirectoryFromFile() {
    std::ifstream inputFile("phone_directory.txt");
    if (inputFile.is_open()) {
        inputFile >> currentRecords;
        for (int i = 0; i < currentRecords; ++i) {
            inputFile >> phoneDirectory[i].id
                >> phoneDirectory[i].lastName
                >> phoneDirectory[i].firstName
                >> phoneDirectory[i].middleName
                >> phoneDirectory[i].birthDate.day
        ...
        inputFile.close();
        std::cout << "Телефонный справочник успешно загружен из файла." << std::endl;
    }
    else {
        std::cout << "Файл с телефонным справочником не найден. Создан новый справочник." << std::endl;
        savePhoneDirectoryToFile();
    }
}
void cleanup() {
    remove("phone_directory.txt");
    savePhoneDirectoryToFile();
}
int main() {
